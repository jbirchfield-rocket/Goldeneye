...
const fetchStones = async () => {
  try {
    ...
    console.log('Available stones:', response.data);
    
    rings.value.forEach(ring => {
      ring.ringStones = response.data;
    });
  } catch (error) {
    console.error('Error fetching stones:', error);
    // Mock data
    const mockStones: Stones[] = [
    ...
    console.log('Mock stones:', mockStones);
  }
}
...
const fetchMaterials = async () => {
  try {
    ...
    rings.value.forEach(ring => {
      ring.materialTypes = response.data;
    });
  } catch (error) {
...
const fetchRings = async () => {
  try {
    const response = await axios.get(`${import.meta.env.VITE_API_URL}/rings`);
