...
public class LocationService {
    ...
    public List<LocationDTO> getLocationsByCustId(int custId) {
        ...
        if (locations.isEmpty()) {
            logger.warn("No locations found for customer ID: {}", custId);
            throw new ResourceNotFoundException("No locations found for customer ID: " + custId);
        }
    ...
    public LocationDTO getLocationById(int locId) {
        ...
        return locationRepo.findByLocId(locId)
            ...
            .orElseThrow(() -> {
                ...
                return new ResourceNotFoundException("Location not found with ID: " + locId);
            });
    }

    public void deleteLocation(int locId) {
        logger.info("Deleting location with ID: {}", locId);
        if (locationRepo.findByLocId(locId).isEmpty()) {
            logger.warn("Delete failed: location not found with ID: {}", locId);
            throw new ResourceNotFoundException("Location not found with ID: " + locId);
        }
    ...
    public int addLocation(LocationDTO location) {
        ...
        if (location.getStreet() == null || location.getStreet().isBlank()) {
            logger.warn("Invalid street provided for customer ID: {}", location.getCustId());
            throw new InvalidLocationException("Street must not be blank.");
        }

        if (location.getCity() == null || location.getCity().isBlank()) {
            logger.warn("Invalid city provided for customer ID: {}", location.getCustId());
            throw new InvalidLocationException("City must not be blank.");
        }

        if (location.getState() == null || location.getState().isBlank()) {
            logger.warn("Invalid state provided for customer ID: {}", location.getCustId());
            throw new InvalidLocationException("State must not be blank.");
        }
        ...
        if (location.getZip() == null || location.getZip().isBlank()) {
            logger.warn("Invalid zip provided for customer ID: {}", location.getCustId());
            throw new InvalidLocationException("Zip code must not be blank.");
        }
